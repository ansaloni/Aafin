...
import { X, PiggyBank, LayoutDashboard, History, ChevronRight } from "lucide-react";
...
export function SideDrawer({ open, onClose, onCreateBudget, activeView, onNavigate }: SideDrawerProps) {
          ...
          {section === "menu" ? (
            <div className="py-3">
              <div className="px-3 mb-3">
                ...
                <button
                  onClick={() => { onNavigate("home"); handleClose(); }}
                  className={cn(
                ...
                <button
                  onClick={() => { onNavigate("history"); handleClose(); }}
                  className={cn(
                    "flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors",
                    activeView === "history"
                      ? "bg-indigo-50 text-indigo-700"
